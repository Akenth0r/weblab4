...
function outArrayData(item, i, arr)
{
	var string = "";
	if (typeof(item)=="string"){
		if(i==0)
			...
			string+= "<tr><td>"+about[i]+"</td><td>" + item + "</td></tr>";

	}
	else if (Array.isArray(item)){
		if (!Array.isArray(item[0]))
		{
			string += "</table><br><table><tr><td>"+about[i] + "</td><td> "+ /*1 + ": "  + */item[0]+"</td></tr>";
			for(var j = 1; j<item.length; j++){
				string +="<tr><td></td>" + "<td>"  + /*(j+1) +  ": "  + */item[j] + "</td></tr>";
			}
			string += "</table><br>";
		}
		else
		{
			string+="<table><tr><td>"+about[i]+"</td>"+"<td>" + outLiteratureData(item[0]) + "</td></tr>";
			for(var j = 1; j<item.length; j++){
			string+="<tr><td></td>" + "<td>" + outLiteratureData(item[j]) + "</td></tr>";}
			string +="</table>";
		}
	...
	{
		string+="<span id ='jtext'>" + about[i] + "</span><table class = 'history'><tr class = 'history'>";
		for(j in item){
...
function makeTableFromCountry(country)
{
	tab = "<table>";
	//Начальные данные:
	gObj = country.map(outArrayData);
	console.log(gObj);
	//tab += "<tr><th colspan = '2'>" + country[0] + " - " + country[1] + "</tr></th>";
	...
	return tab;
		...
		out.forEach( (info, i, arr) => {el.innerHTML += info + "<br>"} )
		console.log("Script was included");
		}
};
